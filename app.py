...
def draw_organogram(slide, relationships, positions, tree):
    ...
    for rel in relationships:
        ...
        if parent_name in shapes and child_name in shapes:
            ...
            connector = slide.shapes.add_connector(MSO_CONNECTOR.ELBOW, int(from_shape.left + from_shape.width / 2), int(from_shape.top + from_shape.height), int(to_shape.left + to_shape.width / 2), int(to_shape.top))
